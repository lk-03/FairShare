...
void main() {
  ...
  testWidgets('GroupDetailScreen renders header, balance hero, member rail, and debts',
      (tester) async {
    ...
    // Verify Group Name & category
    expect(find.text('Flat 402 - Bangalore'), findsWidgets);
    expect(find.text('House'), findsOneWidget);

    // Verify Balance hero
    expect(find.text('YOUR GROUP BALANCE'), findsOneWidget);
    expect(find.text('+₹600.00'), findsOneWidget);
    expect(find.text('You are owed ₹600.00 in total'), findsOneWidget);
    ...
    // Verify Debts to Settle: Sam Altman owes You ₹600.00
    expect(find.text('DEBTS TO SETTLE'), findsOneWidget);
    expect(find.text('Settle Up'), findsOneWidget);

    // Verify Expenses timeline
